...
class AuthenticationTest {
    ...
    val env = LoginEnv(
        login = {
          Single.just(Response.success(200, UserResponse.fake))
              ...
              .map(::GotLoginResponse)
        },
        navigate = { Effect.from { } }
    )

    val givenModel = LoginForm(Email.fake, Password.fake)
    Asserting.on(update, env, givenModel, scheduler = scheduler) {
      run(ClickedLogin)
      assertThat(model)
          .isEqualTo(Authenticating(Email.fake, Password.fake))
  ...
  fun `authentication - unhappy path`() {
    ...
    Asserting.on(update, env, givenModel, scheduler) {
      ...
      scheduler.advanceTimeBy(100, TimeUnit.MILLISECONDS)
      assertThat(model)
          .isEqualTo(LoginForm(Email.fake, Password.fake, GenericErrorModel.fake.errors.body))
    }
  }
}
